...
#' @examples
#' et_mmp_v0(data = candidates,
#' hlaA = hlaApt, hlaB = hlaBpt, hlaDR = hlaDRpt,
#' abo_freq = ABOpt)
#' @export
et_mmp_v0<-function(data = candidates,
                 hlaA = hlaApt, hlaB = hlaBpt, hlaDR = hlaDRpt,
                 abo_freq = ABOpt){
  ...
  data<-data %>% dplyr::left_join(hlaA %>% dplyr::select(A,freq), by = c("A1" = "A"))
  data<- data %>% dplyr::rename(a1=freq)
  data<-data %>% dplyr::left_join(hlaA %>% dplyr::select(A,freq), by = c("A2" = "A"))
  data<- data %>% dplyr::rename(a2=freq)
  data<-data %>% dplyr::left_join(hlaB %>% dplyr::select(B,freq), by = c("B1" = "B"))
  data<- data %>% dplyr::rename(b1=freq)
  data<-data %>% dplyr::left_join(hlaB %>% dplyr::select(B,freq), by = c("B2" = "B"))
  data<- data %>% dplyr::rename(b2=freq)
  data<-data %>% dplyr::left_join(hlaDR %>% dplyr::select(DR,freq), by = c("DR1" = "DR"))
  data<- data %>% dplyr::rename(dr1=freq)
  data<-data %>% dplyr::left_join(hlaDR %>% dplyr::select(DR,freq), by = c("DR2" = "DR"))
  data<- data %>% dplyr::rename(dr2=freq)
  data<-data %>% dplyr::left_join(abo_freq, by = c("bg" = "abo"))
  data<- data %>% dplyr::rename(abo=freq)
...
et_mmp_v1<-function(data = candidates,
  ...
  data[, `:=`(
    ...
    MMP2 = (((2 * (a1 + a2) * (1 - a1 - a2)) - a1 ^ 2 - a2 ^ 2 + SallA) / ((a1 + a2) ^ 2))
         + (((2 * (b1 + b2) * (1 - b1 - b2)) - b1 ^ 2 - b2 ^ 2 + SallB) / ((b1 + b2) ^ 2))
         + (((2 * (dr1 + dr2) * (1 - dr1 - dr2)) - dr1 ^ 2 - dr2 ^ 2 + SallDR) / ((dr1 + dr2) ^ 2))
    ),
    by = 'ID'
  ]
